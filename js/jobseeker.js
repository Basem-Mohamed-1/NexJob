...
function loadAllJobs() {
  ...
  if (openJobs.length === 0) {
    ...
    emptyState.style.cssText =
      "text-align: center; padding: 40px; background: var(--card-bg); border-radius: 12px;";
...
function createJobCard(job) {
  ...
  card.innerHTML = `
    <div class="job-header">
      <div class="job-icon" style="display: flex; align-items: center; justify-content: center; background: var(--primary); color: white; font-weight: bold; font-size: 20px;">${companyInitial}</div>
  ...
  if (myApps.length === 0) {
    container.innerHTML = `
      <div style="text-align: center; padding: 40px; grid-column: 1/-1;">
        <i class="fa-solid fa-file-alt" style="font-size: 48px; color: var(--text-secondary); margin-bottom: 20px;"></i>
        <h3>No Applications Yet</h3>
        <p>Start applying to jobs to track your applications here.</p>
        <a href="findJob.html" class="btn-primary" style="display: inline-block; margin-top: 20px;">Browse Jobs</a>
      </div>
    `;
    return;
  }
